...
final class ProfileViewController: BaseUITableViewController, StoryboardInitiable {
    ...
    fileprivate func setupLoadersCallback() {
        ...
        tableView.addPullToRefresh { [weak self] in
            ...
            this.timeoutTimer = Timer(timeInterval: Constants.Network.timeoutTimeInterval, target: self, selector: #selector(ProfileViewController.noConnection), userInfo: nil, repeats: false)
                ...
                activityService.unfollowUserEventually(user) { [weak self] success, error in
                    if success {
                        ...
                        NotificationCenter.default.post(name: 
                            NSNotification.Name(rawValue: Constants.NotificationName.followersListIsUpdated),
                            object: nil
                        )
                    } else {
